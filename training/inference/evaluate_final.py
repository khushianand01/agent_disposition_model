...
TEST_DATA_PATH = "data/production/test_best.json"
OUTPUT_REPORT = "outputs/qwen_3b_production_best/evaluation_report.json"
...
def evaluate_final():
    ...
    temp_results_path = "outputs/qwen_3b_production_best/temp_results.jsonl"
    ...
    for row in results:
        ...
        # Rule 1: If Payment Disposition is PTP, Amount and Date should not be null/None
        pay_disp = str(pred.get("payment_disposition", "")).upper()
        if pay_disp == "PTP":
            amt = pred.get("ptp_amount")
            date_val = pred.get("ptp_date")
            # Check if they are missing or "None" string
            if not amt or str(amt).lower() == "none": is_consistent = False
            if not date_val or str(date_val).lower() == "none": is_consistent = False
            
        # Rule 2: If Disposition is WRONG_NUMBER, Payment Disposition should be None
        disp = str(pred.get("disposition", "")).upper()
        if disp == "WRONG_NUMBER" and pay_disp != "NONE":
            is_consistent = False
    ...
    if consistency_total > 0:
        print(f"Logical Consistency:      {consistency_passed} ({consistency_passed/consistency_total:.1%})")
    print("-" * 50)
...
if __name__ == "__main__":
    if not os.path.exists("outputs/qwen_3b_production_best"):
        # For testing purposes, we might want to run even without model if we mock?
        # But usually valid.
        print("WARNING: Model not found at outputs/qwen_3b_production_best.")
